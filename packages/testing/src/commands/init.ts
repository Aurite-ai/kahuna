  ...
  try {
    ...
    return {
      ...
      defaults: {
        ...
        framework: parsed?.defaults?.framework ?? DEFAULT_CONFIG.defaults.framework,
...
export async function initCommand(): Promise<void> {
  ...
  try {
    ...
    const email = await prompt(rl, "What's your email? (optional)", existingConfig?.tester.email);
    ...
    // API URL
    console.log('');
    const apiUrl = await prompt(rl, 'API URL', existingConfig?.api.url || DEFAULT_CONFIG.api.url);
