...
| [copilot-configuration.md](./copilot-configuration.md) | Implementers | Files created by `kahuna_setup` |
...
└── context/
    ...
    ├── [curated]/          # Initial patterns from kahuna_setup (e.g., langgraph/)
    └── task/               # Task-specific context (replaced per task)
```

**Initial structure:** `kahuna_setup` pre-populates `context/` with curated patterns (e.g., `langgraph/` folder with best practices). This provides Day 1 value.
...
Six tools in three categories. See [tool-specifications.md](./tool-specifications.md) for detailed specs.

| Category | Tools | Purpose |
|----------|-------|---------|
| **Building Knowledge Base** | `kahuna_learn`, `kahuna_sync` | Populate ~/.kahuna with knowledge |
| **Environment Setup** | `kahuna_setup`, `kahuna_prepare_context` | Prepare project for development |
| **Assistance** | `kahuna_ask`, `kahuna_review` | Use context + KB to help copilot |
