...
```
  ...
  Task desc. ──────►│ kahuna_prepare_context│──── Retrieval Agent (Haiku) ──► selects KB files
                    └───────────────────────┘         │
                                                      ▼
                                               Context Writer ──► project/context/*.md + README.md
...
| `kahuna_prepare_context` | Task start (once) | Writes `.md` files to `project/context/` | Haiku (retrieval) |
...
| D8 | **`context/` gets clean `.md` files** (frontmatter stripped) | Simple, copilot-friendly. Metadata lives in KB. |
| D9 | **`ask` searches KB directly**, not `context/` | If it's in `context/`, copilot already has it. Ask handles what's *not* there. |
| D10 | **`ask` agent told which KB files are in `context/`** | Agent knows what copilot already has; avoids redundancy. |
...
| D14 | **Full clear of `context/`** on each prepare_context call | Simple. Preserve user files later if needed. |
...
The surfacing module handles `context/` folder operations:

1. **Clear** — Remove all files in `context/`
2. **Write files** — For each selected slug: read `.mdc`, strip frontmatter, write as `context/{slug}.md`
3. **Generate README.md** — File table with "Why Relevant" column + "Start Here" section
